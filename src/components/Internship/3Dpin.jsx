...
export const PinPerspective = ({
  ...
  return (
    <motion.div
      className="w-55 h-55 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 transition duration-500">
      <div className=" w-full h-full flex-none  inset-0">
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <a
            href={href}
            target={"_blank"}
            className="relative flex items-center z-10 rounded-full text-center bg-blue-950 py-1.5 px-4 ring-1 ring-white/10 ">
            <span
              className="relative z-20 text-white text-xs font-bold inline-block text-center py-0.5">
              {title}
            </span>

            <span
              className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-emerald-400/0 via-sky-500/90 to-emerald-400/0 transition-opacity duration-500 group-hover/btn:opacity-40"></span>
          </a>
        </div>
        ...
        <div
          ...
          <>
            <motion.div
              ...
              className="absolute left-1/2 top-1/2  h-[11.25rem] w-[11.25rem] rounded-[50%] bg-blue-500/[0.5] shadow-[0_8px_16px_rgb(255_165_0/0.4)]"></motion.div>
            <motion.div
              initial={{
                opacity: 0,
                scale: 0,
